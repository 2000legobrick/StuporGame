...
public class Render implements Runnable {
	...
	public void RenderForeground(Graphics g, int width, int height, int tileSize, ArrayList<Mob> entities, Mob player) {
		...
		for (NewRectangle rect : DisplayedObjects) {
			try {
				if (rect.type == 1) {
					g.drawImage(img, rect.rect.x - player.currentX - player.width / 2 + width / 2,
							rect.rect.y - player.currentY - player.height / 2 + height / 2, rect.rect.width,
							rect.rect.height, null);
					/*
					 * g.setColor(GetColorArray(currentWorld)[rect.type]); g.fillRect(rect.rect.x -
					 * player.currentX - player.width/2 + width / 2, rect.rect.y - player.currentY -
					 * player.height/2 + height/2, rect.rect.width, rect.rect.height);
					 */
				} else if (rect.type != 0) {
		...
		for (NewRectangle rect : DisplayedMobs) {
			...
			if (rect.image != null) {
				...
				g.drawImage(rect.image, rect.rect.x - player.currentX - player.width / 2 + width / 2,
						rect.rect.y - player.currentY - player.height / 2 + height / 2, rect.rect.width,
						rect.rect.height, null);
			} else {
				g.setColor(rect.color);
